...
parser = argparse.ArgumentParser(
    prog="make_bids.py", description="Make BIDS directory structure"
)
parser.add_argument(
    "-d",
    "--debug",
...
parser.add_argument(
    "-dest", "-dt", help="Destination directory", default=".", nargs="?"
)
parser.add_argument(
    "-project", "-p", help="Project name", default="bids_folder", nargs="?"
)
parser.add_argument(
    "-subject_regex",
    "-rx",
    help="Regex pattern to find subject ID strings",
    default="*",
    nargs="?",
)
    ...
    for root, sub_folder, files in os.walk(source_folder):
        for folder_name in sub_folder:
            ...
            if is_modality(folder_name) and is_ID(potential_id, subject_regex):
                ...
                # do not create a modality folder since shutil will do it automatically
                # check_and_create_folder(subject_modality_folder, dry_run)

                original_path = os.path.join(root, folder_name, folder_name)
...
def make_description(dest_folder, dry_run, override_dict=None):

    description_dict = DATASET_DESCRIPTION
    if override_dict is not None:
        description_dict.update(override_dict)

    with open(os.path.join(dest_folder, "dataset_description.json"), "w") as f:
        if not dry_run:
            json.dump(description_dict, f)
        else:
            print("Would have created dataset_description.json")
...
def main():
    ...
    """source_to_dest_mapping = make_bids_folders(
        source_folder, dest_folder, project_name, subject_regex, dry_run
    )
    move_folders(source_to_dest_mapping, dry_run)"""

    make_description(dest_folder, dry_run)
